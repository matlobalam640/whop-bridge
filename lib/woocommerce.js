...
function wcConfig() {
  const baseURL = sanitizeUrl(process.env.WOOCOMMERCE_URL);
...
function storeUrlDiagnostics() {
  const raw = process.env.WOOCOMMERCE_URL || '';
  const clean = sanitizeUrl(raw);
  return {
    store_url: clean,
    store_url_has_whitespace: raw !== raw.trim() || /\s/.test(raw.replace(/https?:\/\//, '')),
